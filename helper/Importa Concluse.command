#!/bin/bash
# Doppio click su questo file per importare lo ZIP "Concluse" nei Cedolini Lavoratori.
# Funziona sia dalla cartella helper/ sia copiato altrove (es. sul Desktop).
DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT="$DIR/import-concluse.mjs"
if [ ! -f "$SCRIPT" ]; then
  # Fallback: copiato fuori dal repo → usa il percorso noto del progetto.
  SCRIPT="$HOME/Documents/GitHub/payslip-scanner/helper/import-concluse.mjs"
fi
if [ ! -f "$SCRIPT" ]; then
  echo "Non trovo import-concluse.mjs. Tieni questo file accanto allo script, oppure"
